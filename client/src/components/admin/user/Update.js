...
import { userFindApi, userUpdateApi } from "../../../service/serviceApi";
...
const Update = (props) => {
  ...
  useDelayCallback(() => {
    getCurrentData(props.userId);
  }, [props.userId]);

  const getCurrentData = (id) => {
    userFindApi(id, []).then((res) => {
      if (res.data.success) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setUserInput({ ...userInput, error_list: [] });
    const data = {
      name: userInput.name,
    ...
    userUpdateApi(props.userId, data).then((res) => {
      if (res.data.success) {
        if (res.data.status === "success") {
          swal({
            title: "Success",
